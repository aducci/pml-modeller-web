...
export const ProcessWorkspaceView = ({ controller, state, onNavigateAdmin, mode = 'editor', onModeChange, aiAssistantPanel, files, activeFileId, onSelectFile, onCreateFile, }) => {
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', width: '100%', height: '100%', overflow: 'hidden' }, children: [_jsxs("header", { style: {
                    ...
                    background: '#fff', borderBottom: '1px solid #E5E7EB', padding: '0 16px', gap: 8,
                        ...
                        }, children: [_jsx(SlidersHorizontal, { size: 12 }), "Admin", hasOverrides && (_jsx("span", { style: { fontSize: 10, background: '#6366F1', color: '#fff', borderRadius: 8, padding: '0 4px', lineHeight: '16px' }, children: "\u2022" }))] })] }), _jsx("div", { style: { display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden' }, children: mode === 'ai-assistant' && aiAssistantPanel ? (
                // Same autoSaveId as the editor-mode PanelGroup below — the left
                // slot's content differs (AI chat vs. PML editor) but it's the
                // same left/right split, so switching tabs should feel like one
                // continuous splitter, not two that each forget their own size.
                // react-resizable-panels persists this to localStorage keyed by
                // autoSaveId; matching `id`s on the panels are what let it apply a
                // saved size back to the correct pane after a remount.
                _jsxs(PanelGroup, { autoSaveId: "pml-workspace-left-split", direction: "horizontal", style: { width: '100%', height: '100%' }, children: [_jsx(Panel, { id: "workspace-left", order: 1, defaultSize: 28, minSize: 18, children: aiAssistantPanel }), _jsx(PanelResizeHandle, { children: _jsx("div", { style: { width: 5, height: '100%', cursor: 'col-resize', background: '#E5E7EB', transition: 'background 0.15s' }, onMouseEnter: e => (e.currentTarget.style.background = '#6366F1'), onMouseLeave: e => (e.currentTarget.style.background = '#E5E7EB') }) }), _jsx(Panel, { id: "workspace-right", order: 2, defaultSize: 72, minSize: 25, children: _jsxs("div", { style: { position: 'relative', width: '100%', height: '100%' }, children: [_jsx(ProcessCanvasView, { state: state, onZoom: (z) => controller.setZoom(z), onPan: (dx, dy) => controller.pan(dx, dy), onSelect: (type, id) => controller.selectElement(type, id), onSetViewport: (zoom, panX, panY) => controller.setViewport(zoom, panX, panY), onResetView: () => controller.resetView(), onToggleLanes: () => {
