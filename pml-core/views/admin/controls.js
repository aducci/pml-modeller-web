...
export const Field = ({ label, hint, children, align = 'end', reverseLayout = false }) => {
    const labelBlock = (_jsxs("div", { children: [_jsx("div", { style: { fontSize: 12, fontWeight: 500, color: '#374151' }, children: label }), hint && _jsx("div", { style: { fontSize: 11, color: '#9CA3AF', marginTop: 1 }, children: hint })] }));
    const controlBlock = (_jsx("div", { style: { display: 'flex', alignItems: 'center', justifyContent: reverseLayout ? 'flex-start' : (align === 'start' ? 'flex-start' : 'flex-end') }, children: children }));
    return (_jsxs("div", { style: {
            display: 'grid',
            gridTemplateColumns: reverseLayout ? 'auto 1fr' : '1fr auto',
            alignItems: 'center', gap: '8px 16px', padding: '7px 0', borderBottom: '1px solid #F1F0ED',
        }, children: [reverseLayout ? controlBlock : labelBlock, reverseLayout ? labelBlock : controlBlock] }));
};
