...
export default function ProcessCanvas({ layoutResult, zoom, panX, panY, viewportWidth, viewportHeight, theme, interactionMode = 'select', onZoomRequest, onPanRequest, selectedElement, onElementSelect, onElementDoubleClick, showLanes = true, viewAsActor, highlightNodeIds, flowVisibility, connectorStyle = 'flowTypes', curtainsOn = true, }) {
    ...
    const handleLaneDoubleClick = useCallback((laneId, e) => {
        if (e.target.closest('button, input, select'))
            return;
        e.stopPropagation();
        onElementDoubleClick?.('lane', laneId);
    }, [onElementDoubleClick]);
    // Curtains (inbound/outbound boundary bands) had no click handling at
    // all previously — the only way to reach curtain styling in the admin
    // Theme panel was indirectly, via clicking an inbound/outbound event node.
    const handleCurtainClick = useCallback((side, e) => {
        if (e.target.closest('button, input, select'))
            return;
        e.stopPropagation();
        onElementSelect?.('curtain', side);
    }, [onElementSelect]);
    ...
    const handleNodeDoubleClick = useCallback((nodeId, e) => {
        if (shouldIgnoreSelectionClick(e))
            return;
        e.stopPropagation();
        onElementDoubleClick?.('node', nodeId);
    }, [onElementDoubleClick, shouldIgnoreSelectionClick]);
    return (_jsxs("svg", { ref: svgRef, width: viewportWidth, height: viewportHeight, viewBox: `0 0 ${viewportWidth} ${viewportHeight}`, style: {
            ...
            touchAction: 'none',
            userSelect: 'none',
        }, onWheel: handleWheel, onPointerDown: handlePointerDown, onPointerMove: handlePointerMove, onPointerUp: handlePointerUp, onPointerCancel: handlePointerUp, children: [_jsxs("defs", { children: [_jsx("marker", { id: "edgeArrow", markerWidth: "10", markerHeight: "10", refX: "9", refY: "3", orient: "auto", children: _jsx("polygon", { points: "0 0, 10 3, 0 6", fill: themeEdges.marker.fill }) }), _jsx("marker", { id: "edgeArrowOpen", markerWidth: "10", markerHeight: "10", refX: "9", refY: "3", orient: "auto", children: _jsx("polygon", { points: "0 0, 10 3, 0 6", fill: "#fff", stroke: themeEdges.marker.openStroke ?? themeEdges.marker.stroke ?? themeEdges.marker.fill, strokeWidth: "1" }) }), _jsx("filter", { id: "nodeShadow", x: "-20%", y: "-20%", width: "140%", height: "140%", children: _jsx("feDropShadow", { dx: "0", dy: "1", stdDeviation: "1.2", floodColor: "#0f172a", floodOpacity: "0.10" }) }), _jsx("pattern", { id: "grid", width: "24", height: "24", patternUnits: "userSpaceOnUse", children: _jsx("circle", { cx: "0.5", cy: "0.5", r: "1", fill: "#CBD5E1", opacity: "0.55" }) })] }), _jsxs("g", { transform: `translate(${panX} ${panY}) scale(${zoom})`, children: [_jsx("rect", { x: "0", y: "0", width: renderBounds.width, height: renderBounds.height, fill: "url(#grid)" }), curtainsOn && scene.curtains.map((curtain) => renderCurtain(curtain, handleCurtainClick)), scene.lanes.map((lane) => renderLaneModel(lane, selectedElement?.id, handleLaneClick, handleLaneDoubleClick)), scene.edges.map((edge) => renderEdgeModel(edge, handleEdgeClick)), scene.nodes.map((node) => renderNodeModel(node, hoveredNodeId, setHoveredNodeId, handleNodeClick, handleNodeDoubleClick, resolvedTheme.nodeEffects)), _jsx("rect", { x: scene.visualBounds.x, y: scene.visualBounds.y, width: scene.visualBounds.width, height: scene.visualBounds.height, fill: "none", stroke: themeLanes.borderColor, strokeWidth: "1", strokeDasharray: "5,5", opacity: "0.4", rx: "8" })] })] }));
}
// ============================================================================
// Lane renderer
// ============================================================================
function renderLaneModel(lane, selectedLaneId, onLaneClick, onLaneDoubleClick) {
    ...
    return (_jsxs("g", { onClick: (e) => onLaneClick?.(lane.id, e), onDoubleClick: (e) => onLaneDoubleClick?.(lane.id, e), className: "cursor-pointer transition-opacity hover:opacity-90", children: [_jsx("rect", { x: lane.x, y: lane.y, width: lane.width, height: lane.height, fill: lane.bodyFill, stroke: borderColor, strokeWidth: borderWidth, rx: lane.cornerRadius }), _jsx("rect", { x: lane.x, y: lane.y, width: lane.width, height: lane.headerHeight, fill: lane.headerFill, stroke: lane.headerBorderColor, strokeWidth: borderWidth, rx: lane.cornerRadius }), lane.label && (_jsx("text", { x: lane.x + 10, y: lane.y + lane.headerHeight / 2, fontSize: lane.headerHeight * 0.45, fontWeight: lane.headerFontWeight, fill: lane.labelColor, opacity: lane.headerLabelOpacity, dominantBaseline: "middle", children: lane.label }))] }, lane.id));
}
// ============================================================================
// Curtain renderer
// ============================================================================
function renderCurtain(curtain, onCurtainClick) {
    return (_jsxs("g", { onClick: (e) => onCurtainClick?.(curtain.side, e), style: { cursor: onCurtainClick ? 'pointer' : undefined }, children: [_jsx("rect", { x: curtain.x, y: curtain.y, width: curtain.width, height: curtain.height, fill: curtain.fill, fillOpacity: curtain.fillOpacity, stroke: curtain.stroke, strokeWidth: curtain.strokeWidth, rx: 4 }), _jsx("text", { x: curtain.labelX, y: curtain.labelY, textAnchor: "middle", fontSize: 14, fontWeight: 600, fill: curtain.labelColor, children: curtain.label })] }, curtain.side));
}
// ============================================================================
// Edge renderer
// ============================================================================
...
// Shape body
// ============================================================================
function renderShape(node, nodeEffects) {
    const { x, y, width, height, shape } = node;
    const cx = x + width / 2;
    const cy = y + height / 2;
    const shadowFilter = nodeEffects?.shadow !== false ? 'url(#nodeShadow)' : undefined;
    const gradientId = `nodeGrad-${node.id}`;
    const fill = nodeEffects?.gradient ? `url(#${gradientId})` : shape.fill;
    const gradientDef = nodeEffects?.gradient ? (_jsxs("linearGradient", { id: gradientId, x1: "0", y1: "0", x2: "0", y2: "1", children: [_jsx("stop", { offset: "0", stopColor: lightenHexColor(shape.fill, 0.18) }), _jsx("stop", { offset: "1", stopColor: shape.fill })] })) : null;
    if (shape.kind === 'circle') {
        return (_jsxs(_Fragment, { children: [gradientDef && _jsx("defs", { children: gradientDef }), _jsx("circle", { cx: cx, cy: cy, r: Math.min(width, height) / 2, fill: fill, stroke: shape.stroke, strokeWidth: shape.strokeWidth, strokeDasharray: shape.strokeDasharray, filter: shadowFilter })] }));
    }
    if (shape.kind === 'diamond') {
        return (_jsxs(_Fragment, { children: [gradientDef && _jsx("defs", { children: gradientDef }), _jsx("polygon", { points: `${cx},${y} ${x + width},${cy} ${cx},${y + height} ${x},${cy}`, fill: fill, stroke: shape.stroke, strokeWidth: shape.strokeWidth, strokeDasharray: shape.strokeDasharray, filter: shadowFilter })] }));
    }
    return (_jsxs(_Fragment, { children: [gradientDef && _jsx("defs", { children: gradientDef }), _jsx("rect", { x: x, y: y, width: width, height: height, rx: shape.cornerRadius ?? 6, fill: fill, stroke: shape.stroke, strokeWidth: shape.strokeWidth, strokeDasharray: shape.strokeDasharray, filter: shadowFilter })] }));
}
/** Lightens a `#rrggbb` hex color toward white by `amount` (0-1). Falls back to the input for non-hex fills (gradients, named colors). */
