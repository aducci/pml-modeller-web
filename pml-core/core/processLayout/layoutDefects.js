...
// connect to.
import { forEachSegmentCrossing } from './convergenceLoop';
import { nodeRect, rectBounds } from '../layoutGeometry';
function nodeOverlapsSegment(node, a, b) {
    if (node.x === undefined || node.y === undefined)
        return false;
    const { minX: left, minY: top, maxX: right, maxY: bottom } = rectBounds(nodeRect(node));
...
export function countLayoutDefects(nodes, edges) {
    ...
    forEachSegmentCrossing(routed, (a, b) => a.source === b.source || a.source === b.target || a.target === b.source || a.target === b.target, (a, b) => {
        count += 1;
