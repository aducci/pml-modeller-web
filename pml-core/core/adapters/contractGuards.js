...
export const DEFAULT_RULE_CONFIGS = [
    {
        code: 'OUTBOUND_HAS_OUTGOING',
        shape: 'edgeCount',
        enabled: true,
        category: 'structural',
        predicate: 'isOutboundEvent',
        direction: 'outgoing',
        operator: 'gt',
        threshold: 0,
        messageTemplate: 'Outbound event "{nodeId}" has an outgoing edge — outbound events are terminal and must not lead anywhere else. Model the downstream step as a task instead, or split into two parallel edges from the source task.',
    },
    {
        ...
        enabled: true,
        category: 'structural',
        predicate: 'isInboundEvent',
        direction: 'incoming',
        operator: 'gt',
        threshold: 0,
        messageTemplate: 'Inbound event "{nodeId}" has an incoming edge — inbound events are entry points and must not be led into by another step. Start the flow at this event instead of routing something into it.',
    },
    {
        ...
        enabled: true,
        category: 'semantic',
        predicate: 'isNotGateway',
        direction: 'outgoing',
        operator: 'gt',
        threshold: 1,
        messageTemplate: '"{nodeId}" has {count} outgoing edges but isn\'t a gateway. If these happen concurrently, model as an explicit "decision(AND)" gateway (branches render as equally-weighted main flow, not one main + alternates); if they\'re mutually exclusive, model as a "decision" with named outcomes.',
    },
    {
        code: 'NODE_ORPHANED',
        shape: 'edgeCount',
        enabled: true,
        category: 'structural',
        nodeTypes: ['task', 'decision', 'event'],
        direction: 'both',
        operator: 'eq',
        threshold: 0,
        messageTemplate: '{nodeType} "{nodeId}" has no incoming or outgoing edges — it isn\'t connected to the rest of the process yet.',
    },
    {
        ...
        operator: 'eq',
        threshold: 1,
        messageTemplate: 'Decision "{nodeId}" has only one outcome ("{firstOutcome}") — a decision with a single branch usually means the alternate or exception path hasn\'t been modelled yet.',
    },
    {
        code: 'TASK_NO_ACTOR',
        shape: 'fieldMissing',
        ...
        nodeType: 'task',
        field: 'actor',
        messageTemplate: 'Task "{nodeId}" has no actor assigned.',
    },
    {
        ...
        category: 'semantic',
        field: 'actor',
        onlySemanticRole: 'normalFlow',
        messageTemplate: 'Edge "{source} > {target}" crosses from {sourceValue} to {targetValue} with no semanticRole set. If this represents a message/handoff between actors, consider tagging it semanticRole=messageFlow; if it\'s just an ordinary next step, this can be left as-is.',
    },
];
