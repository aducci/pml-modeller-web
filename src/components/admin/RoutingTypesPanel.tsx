...
import { deriveRoutingTypeCode } from 'pml-core';
import { ChevronDown, ChevronRight } from 'lucide-react';

// ---------------------------------------------------------------------------
// Mini SVG diagram renderer
// ---------------------------------------------------------------------------

interface MiniDiagramProps {
  // Waypoints in a normalised 0-100 coordinate space
...
function MiniDiagram({ points, isDashed, width = 96, height = 60 }: MiniDiagramProps) {
  ...
  return (
    <svg
      width={width}
      height={height}
      viewBox={`0 0 100 ${Math.round(100 * height / width)}`}
      style={{ display: 'block', overflow: 'visible' }}
    >
      {/* Source node */}
      <rect x={sx - 10} y={sy - 5} width={20} height={10} rx={2}
        fill="#EEF2FF" stroke="#6366F1" strokeWidth={1.5} />
      {/* Target node */}
      <rect x={ex - 10} y={ey - 5} width={20} height={10} rx={2}
        fill="#EEF2FF" stroke="#6366F1" strokeWidth={1.5} />
      {/* Path */}
      <polyline
        points={ptStr}
        fill="none"
        stroke={isDashed ? '#EA580C' : '#6366F1'}
        strokeWidth={2}
      ...
      />
      {/* Arrow head */}
      <polygon points={arrow} fill={isDashed ? '#EA580C' : '#6366F1'} />
    </svg>
  );
}
...
export interface RoutingTypeDef {
  ...
  skew: boolean;
  isDashed: boolean;
  // Waypoints in 0-100 normalised space (viewBox "0 0 100 62")
...
const ROUTING_TYPES: RoutingTypeDef[] = [
  // ── Straight ──────────────────────────────────────────────────────────────
  {
    code: 'STH',
...
export const RoutingTypesPanel: React.FC<Props> = ({ layoutResult }) => {
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 15, fontWeight: 700, color: '#111827', margin: 0 }}>Connection Routing Types</h2>
        <p style={{ fontSize: 12, color: '#6B7280', margin: '4px 0 0', lineHeight: 1.5 }}>
          Reference for all 14 canonical routing types — code, geometry, trigger conditions, and internal engine mapping.
          {layoutResult && (
            <span style={{ color: '#059669', marginLeft: 4 }}>Live counts reflect the current diagram.</span>
          )}
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 20 }}>
        <FilterChip label="All types" active={filterGroup === null} onClick={() => setFilterGroup(null)} />
        {GROUPS.map(g => (
          <FilterChip key={g} label={g} active={filterGroup === g} onClick={() => setFilterGroup(filterGroup === g ? null : g)} color={GROUP_COLORS[g].badge} />
        ))}
      </div>

      {/* Table */}
      <div style={{ border: '1px solid #E5E7EB', borderRadius: 8, overflow: 'hidden', marginBottom: 28 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '52px 80px 1fr 90px 60px 80px 72px', padding: '8px 14px', background: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
          <span />
          <span style={thStyle}>Diagram</span>
          <span style={thStyle}>Full name</span>
          <span style={thStyle}>Anchors</span>
          <span style={thStyle}>Bends</span>
          <span style={thStyle}>Skew</span>
          <span style={thStyle}>{layoutResult ? 'Live' : ''}</span>
        </div>
        {filtered.map(t => (
          <SummaryRow
            key={t.code}
            type={t}
            isExpanded={expandedCode === t.code}
            onToggle={() => setExpandedCode(prev => prev === t.code ? null : t.code)}
            liveCount={liveCountByCode[t.code]}
            showLive={Boolean(layoutResult)}
          />
        ))}
      </div>

      {/* Skew / ElbowYPolicy reference */}
      <RefTable
        title="Skew → ElbowYPolicy mapping"
        subtitle="How the right-pane skew label maps to the engine's internal ElbowYPolicy."
        cols={['Skew', 'ElbowYPolicy', 'Effect on elbow position']}
        rows={[
      ...
      <div style={{ border: '1px solid #E5E7EB', borderRadius: 8, overflow: 'hidden', marginBottom: 28 }}>
        <div style={{ padding: '12px 16px', background: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, color: '#374151', margin: 0 }}>Classification decision tree</h3>
          <p style={{ fontSize: 11, color: '#9CA3AF', margin: '2px 0 0' }}>Priority order used by deriveRoutingTypeCode (and scenarioResolver.ts upstream).</p>
        </div>
        <div style={{ padding: '14px 16px' }}>
          <pre style={{ fontSize: 11, color: '#374151', margin: 0, lineHeight: 1.8, fontFamily: 'monospace' }}>{`1.  source.id === target.id                       → SLP
...
NOTE: cross-lane-upward is NOT a back-route. It classifies as DEH/DEF/DEN
      based on skew. Only scenarioKey "loopback-*" produces TEH/TEV.`}</pre>
        </div>
      </div>
    </div>
  );
    ...
    <>
      ...
      >
        ...
        <div style={{ paddingRight: 8 }}>
          <MiniDiagram points={type.miniPoints} isDashed={type.isDashed} width={72} height={44} />
        </div>

        {/* Name */}
        <div>
          <span style={{ fontSize: 12, color: '#374151' }}>{type.fullName}</span>
          {type.isDashed && (
            <span style={{ marginLeft: 6, fontSize: 10, color: '#D97706', background: '#FEF3C7', border: '1px solid #FCD34D', borderRadius: 3, padding: '0 4px' }}>dashed</span>
          )}
        </div>

        {/* Anchors */}
        <span style={{ fontSize: 10, fontFamily: 'monospace', color: '#6B7280' }}>
          {type.sourceAnchor.split(' ')[0]} → {type.targetAnchor.split(' ')[0]}
        </span>

        {/* Bends */}
        <span style={{ fontSize: 12, color: '#374151', textAlign: 'center' }}>
          {type.bends === -1 ? '?' : type.bends}
        </span>

        {/* Skew */}
        <span style={{ fontSize: 11, color: type.skew ? '#6366F1' : '#D1D5DB' }}>
          {type.skew ? 'near/mid/far' : '—'}
        </span>

        {/* Live count */}
        {showLive && (
          <span style={{ fontSize: 11, fontWeight: 600, textAlign: 'center', color: liveCount ? '#059669' : '#D1D5DB' }}>
            {liveCount ?? 0}
          </span>
        )}
      </div>
      ...
      {isExpanded && (
        <div style={{ padding: '4px 14px 16px', borderBottom: '1px solid #F3F4F6', background: '#FAFAFA' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 20, marginTop: 8 }}>
            {/* Larger diagram */}
            <div>
              <div style={subheadStyle}>Geometry</div>
              <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: 12, display: 'inline-block' }}>
                <MiniDiagram points={type.miniPoints} isDashed={type.isDashed} width={120} height={74} />
              </div>
            </div>

            {/* Detail fields */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 20px' }}>
              <DetailBlock label="Trigger" value={type.trigger} span />
              <DetailBlock label="Source anchor" value={type.sourceAnchor} mono />
              <DetailBlock label="Target anchor" value={type.targetAnchor} mono />
              <DetailBlock label="Internal bendType" value={type.codeMapping.bendType} mono />
              <DetailBlock label="Scenario pattern" value={type.codeMapping.scenarioPattern} mono />
              <DetailBlock label="ElbowYPolicy" value={type.codeMapping.elbowYPolicy} mono />
              {type.notes && <DetailBlock label="Notes" value={type.notes} span />}
            </div>
          </div>
        </div>
      )}
...
function buildLiveCounts(layoutResult: any): Record<string, number> {
  if (!layoutResult?.edges) return {};
  const counts: Record<string, number> = {};
  for (const edge of layoutResult.edges as any[]) {
    const prefs = edge.routing?.preferences;
    if (!prefs?.bendType) continue;
    const info = deriveRoutingTypeCode(
      prefs.bendType,
      prefs.sourcePortSelected ?? prefs.sourceAnchor,
      prefs.targetPortSelected ?? prefs.targetAnchor,
      prefs.portRuleElbowYPolicy ?? 'midpointY',
      edge.routing?.scenario ?? '',
      edge.routing?.channel ?? 0,
      edge.source === edge.target,
    );
    if (info) counts[info.code] = (counts[info.code] ?? 0) + 1;
  }
  return counts;
}
...
function DetailBlock({ label, value, mono, span }: { label: string; value: string; mono?: boolean; span?: boolean }) {
  return (
    <div style={span ? { gridColumn: '1 / -1' } : {}}>
      <div style={subheadStyle}>{label}</div>
      <div style={{ fontSize: 11, color: '#374151', fontFamily: mono ? 'monospace' : undefined, lineHeight: 1.5 }}>{value}</div>
    </div>
  );
}

function RefTable({ title, subtitle, cols, rows }: { title: string; subtitle: string; cols: string[]; rows: string[][] }) {
  return (
    <div style={{ border: '1px solid #E5E7EB', borderRadius: 8, overflow: 'hidden', marginBottom: 28 }}>
      <div style={{ padding: '12px 16px', background: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: cols.map(() => 'auto').join(' 1fr ').replace('auto 1fr auto', 'auto 1fr 1fr') }}>
        {cols.map((c, i) => (
          <div key={i} style={{ padding: '7px 14px', background: '#F9FAFB', fontSize: 10, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.08em', color: '#9CA3AF', borderBottom: '1px solid #E5E7EB' }}>{c}</div>
        ))}
        {rows.map((row, ri) =>
          row.map((cell, ci) => (
            <div key={`${ri}-${ci}`} style={{ padding: '7px 14px', fontSize: 11, color: ci === 0 ? '#6366F1' : '#374151', fontFamily: ci < 2 ? 'monospace' : undefined, background: ri % 2 ? '#F9FAFB' : '#fff', borderBottom: '1px solid #F3F4F6' }}>
              {cell}
            </div>
          ))
        )}
...
const thStyle: React.CSSProperties = { fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9CA3AF' };
const subheadStyle: React.CSSProperties = { fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9CA3AF', marginBottom: 4 };
